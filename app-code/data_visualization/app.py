...
def update_dashboard(n_clicks, symbol, period):
    ...
    try:
        ...
        global GLOBAL_CHANGE_PERCENT
        if len(df) > 1:
           previous_price = df["close"].iloc[-2]
           GLOBAL_CHANGE_PERCENT = ((latest_price - previous_price) / previous_price) * 100
        else:
           GLOBAL_CHANGE_PERCENT = 0.0
        ...
        sentiment_data = generate_sample_sentiment(symbol, df["close"].iloc[-1])
...
def generate_sample_sentiment(symbol, price, change_percent=None):
...
def generate_sample_summary(symbol, current_price, change_percent=None):
